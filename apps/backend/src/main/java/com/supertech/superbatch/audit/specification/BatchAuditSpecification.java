        ...
        return (root, query, cb) -> {
            ...
            if (request.search() != null && !request.search().isBlank()) {
                ...
                predicates.add(cb.or(
                        ...
                        cb.like(cb.lower(root.get("action").as(String.class)), search),
            ...
            if (request.moduleId() != null) {
                predicates.add(cb.equal(root.get("module").get("id"), request.moduleId()));
            }
            if (request.action() != null) {
                predicates.add(cb.equal(root.get("action"), request.action()));
            }
            if (request.userId() != null) {
                predicates.add(cb.equal(root.get("performedBy").get("id"), request.userId()));
            }
            if (request.fromDate() != null) {
                predicates.add(cb.greaterThanOrEqualTo(root.get("performedAt"), request.fromDate().atStartOfDay()));
            }
            if (request.toDate() != null) {
                predicates.add(
                        cb.lessThanOrEqualTo(root.get("performedAt"), request.toDate().plusDays(1).atStartOfDay()));
            }
