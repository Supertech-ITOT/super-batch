...
export const baseUnitSchema = z.object({
  ...
  recipeQuantityType: z
    .string({ error: "Recipe Quantity Type is required." })
    .min(1, "Recipe Quantity Type is required")
    .trim(),
});

export const createUnitSchema = baseUnitSchema;

export const updateUnitSchema = baseUnitSchema.omit({
  recipeQuantityType: true,
});

export type CreateUnitSchema = z.infer<typeof createUnitSchema>;
export type UpdateUnitSchema = z.infer<typeof updateUnitSchema>;

export const unitDefaultValues: CreateUnitSchema = {
  name: "",
  areaId: 0,
  capacity: 0,
  code: "",
