...
export default function CreateRecipeDialog({ open, onClose }: Props) {
  const { mutateAsync: createRecipe, isPending: isCreating } =
    useCreateRecipe();
  ...
  const loading =
    isSubmitting || isCreating || isLoadingMaterials || isLoadingUnits;
  const selectedUnitId = watch("unitId");
  const selectedUnitMaxRange = units?.find(
  ...
  const onSubmit = async (formData: CreateRecipeSchema) => {
    ...
    if (formData.batchSize > selectedUnitMaxRange) {
      toast.error(
        `Batch size must be under unit capacity - ${selectedUnitMaxRange}kg`,
      );
      return;
    }
    try {
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <NumberInput
            ...
            suffix="KG"
            placeholder={
              selectedUnitMaxRange ? `0 - ${selectedUnitMaxRange}` : "0"
            }
            disabled={loading}
