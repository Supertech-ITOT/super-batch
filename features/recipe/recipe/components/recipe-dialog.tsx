...
export default function RecipeCard({ recipeId }: { recipeId?: number }) {
    ...
    const { register, handleSubmit, reset, watch, control, formState: { isSubmitting, isDirty } } = useForm<RecipeSchema>({
        ...
        defaultValues: { stepNo: undefined, stdTime: "", actionId: 0, transitionId: 0, message: "", materials: [], parameters: [] }
    });
                ...
                <CardHeader className="relative overflow-hidden border-b bg-muted/40 py-4!">
                    ...
                    <div className="relative flex items-start gap-4">
                        ...
                        <div>
                            ...
                            <CardDescription>
                                {isEdit ? "Update the step information." : "Create a new process step."}
                            </CardDescription>
                        </div>
                    </div>
                </CardHeader>
                ...
                <CardContent className="min-h-0 flex-1 overflow-y-auto p-4">
                    <Tabs value={tab} className="mt-4">
                        <TabsList className="grid w-full grid-cols-3">
                            {["basic", "material", "parameter"].map((t) => (
                        ...
                        <TabsContent value="basic" className="space-y-6 mt-4">
                            ...
                            <div className="space-y-2 relative">
                                <div className="flex items-center justify-between">
                                    <Label>Message</Label>
                                    <CharacterProgress value={watch("message")} max={RecipeSchemaLimit.message.max} />
                                </div>
                                <Controller
                                    control={control}
                                    name="message"
                                    render={({ field }) => (
                                        <TextareaAutocomplete
                                            value={field.value}
                                            onChange={field.onChange}
                                            options={messages.map((m) => ({ id: m.id, label: m.name, }))}
                            ...
                            <Controller
                                ...
                                render={({ field }) => (
                                    <ValuePicker
                                        ...
                                        valueLabel="Std Qty"
                                        options={materials.filter((f) => f.materialType !== MaterialType.FINISHED_PRODUCT).map((m) => ({ id: m.id, name: m.name, uom: m.uom.symbol }))}
                                        value={(field.value ?? []).map((m) => ({ id: m.id, value: m.stdQty, }))}
                                        onChange={(items) => field.onChange(items.map((i) => ({ id: i.id, stdQty: i.value, })))}
                                    />
                                )}
                            />
                        </TabsContent>
                            ...
                            <Controller
                                ...
                                render={({ field }) => (
                                    <ValuePicker
                                        ...
                                        options={parameters.map((p) => ({ id: p.id, name: p.name, uom: p.uom.symbol }))}
                                        value={(field.value ?? []).map((p) => ({ id: p.id, value: p.stdValue, }))}
                                        onChange={(items) => field.onChange(items.map((i) => ({ id: i.id, stdValue: i.value, })))}
                                    />
                                )}
                            />
