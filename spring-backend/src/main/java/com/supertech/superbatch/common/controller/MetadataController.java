...
public class MetadataController {
    ...
    public ApiResponse<List<OptionDto>> getEquipmentTypes() {
        List<OptionDto> data = Arrays.stream(EquipmentType.values()).map(type -> new OptionDto(
                EnumUtil.formatLabel(type.name()),
                type.name()))
                .toList();
        return new ApiResponse<>(true, "Equipment types fetched successfully", data);
    }

    @GetMapping("/api/metadata/unit-types")
    public ApiResponse<List<OptionDto>> getUnitTypes() {
        List<OptionDto> data = Arrays.stream(UnitType.values()).map(type -> new OptionDto(
                EnumUtil.formatLabel(type.name()),
                type.name()))
                .toList();

        return new ApiResponse<>(true, "Unit types fetched successfully", data);
    }
}